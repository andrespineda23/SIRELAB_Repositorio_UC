...
public class ReservaSalaDAO implements ReservaSalaDAOInterface {
    ...
    public List<ReservaSala> consultarReservaSalasPorFechasYPeriodo(Date fechaInicio, Date fechaFin, BigInteger periodo) {
        try {
            em.clear();
            Query query = em.createQuery("SELECT p FROM ReservaSala p WHERE p.reserva.fechareserva>=:fechaInicio AND p.reserva.fechareserva<=:fechaFin AND p.reserva.periodoacademico.idperiodoacademico=:periodo ORDER BY p.reserva.fechareserva ASC");
            query.setHint("javax.persistence.cache.storeMode", "REFRESH");
            ...
            logger.error("Error consultarReservaSalasPorFechasYPeriodo ReservaSalaDAO : " + e.toString(), e);
            return null;
        }
    }
    ...
    public ReservaSala buscarReservaSalaPorCodigoReserva(String reserva) {
        try {
            em.clear();
            Query query = em.createQuery("SELECT p FROM ReservaSala p WHERE p.reserva.numeroreserva=:reserva");
            query.setHint("javax.persistence.cache.storeMode", "REFRESH");
            query.setParameter("reserva", reserva);
            ReservaSala registro = (ReservaSala) query.getSingleResult();
            return registro;
        } catch (Exception e) {
            logger.error("Error buscarReservaSalaPorCodigoReserva ReservaSalaDAO : " + e.toString(), e);
            return null;
        }
    }
