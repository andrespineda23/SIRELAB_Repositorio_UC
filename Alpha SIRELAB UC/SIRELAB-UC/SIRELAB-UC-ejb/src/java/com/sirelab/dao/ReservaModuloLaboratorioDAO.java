...
public class ReservaModuloLaboratorioDAO implements ReservaModuloLaboratorioDAOInterface {
    ...
    @Override
    public List<ReservaModuloLaboratorio> buscarReservaModuloLaboratorioPorUsuarioYPeriodo(String usuario, BigInteger periodo) {
        try {
            em.clear();
            Query query = em.createQuery("SELECT p FROM ReservaModuloLaboratorio p WHERE p.reserva.persona.identificacionpersona=:usuario AND p.reserva.periodoacademico.idperiodoacademico=:periodo ORDER BY p.reserva.fechareserva ASC");
            query.setHint("javax.persistence.cache.storeMode", "REFRESH");
            query.setParameter("usuario", usuario);
            query.setParameter("periodo", periodo);
            List<ReservaModuloLaboratorio> registro = query.getResultList();
            ...
            logger.error("Error buscarReservaModuloLaboratorioPorUsuarioYPeriodo ReservaModuloLaboratorioDAO : " + e.toString(), e);
            return null;
        }
    }
    @Override
